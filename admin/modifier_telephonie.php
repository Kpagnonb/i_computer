...
$sql_select_produit = "SELECT * FROM telephonie WHERE IdProduit = :id_produit";
...
$sql_select_images = "SELECT id, chemin_image FROM images_telephonie WHERE id_produit = :id_produit";
...
try {
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $sql_update_produit = "UPDATE telephonie SET 
                               Nom = :nom, 
        ...
        if ($stmt_update_produit->rowCount() > 0) {
            foreach ($images_a_supprimer as $image_id) {
                $sql_delete_image = "DELETE FROM images_telephonie WHERE id = :image_id";
                $stmt_delete_image = $pdo->prepare($sql_delete_image);
                $stmt_delete_image->execute(['image_id' => (int)$image_id]);
            }

            foreach ($imagePaths as $imagePath) {
                $sql_insert_image = "INSERT INTO images_telephonie (id_produit, chemin_image) VALUES (:id_produit, :image_path)";
            ...
            $_SESSION['message'] = 'Les détails du téléphonie ont été mis à jour avec succès!';
            header('Location: telephonie.php');
            exit;
        } else {
            echo "Erreur lors de la mise à jour du téléphonie.";
        }
    }
} catch (PDOException $e) {
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-4 text-gray-800">Modifier l'appareil</h1>
                    <form action="modifier_telephonie.php?id=<?php echo $id_produit; ?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <option value="Smartphone" <?php if ($produit['TypeProduit'] == 'Smartphone') echo 'selected'; ?>>Smartphone</option>
                            <option value="Accessoire téléphone" <?php if ($produit['TypeProduit'] == 'Accessoire téléphone') echo 'selected'; ?>>Accessoire téléphone</option>
                            <option value="Objet connecté" <?php if ($produit['TypeProduit'] == 'Objet connecté') echo 'selected'; ?>>Objet connecté</option>
                            <option value="Téléphone fixe" <?php if ($produit['TypeProduit'] == 'Téléphone fixe') echo 'selected'; ?>>Téléphone fixe</option>
                            </select>
                        </div>
                        <div class="form-group">
